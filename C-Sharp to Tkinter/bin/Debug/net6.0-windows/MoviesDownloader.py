...
def Search(searchName):
    ...
    egybest = EgyBest()
    searchResult = egybest.search(searchName)
...
def getNumberOfSeasons(show):
    return len(show.getSeasons())
    
def getNumberOfEpisodes(show, season):
    return len(show.getSeasons()[season].getEpisodes())

def getSeasons(show, quality, seasonNumber, episodeNumber, forceDownload):
    try:
        sStart, sEnd, sIsRanged = getRange(seasonNumber)
        if(sStart < 0):
            return False
    except:
        return False
    if (sIsRanged):
        ...
        StartSeasonsThreading(show.getSeasons(), sStart, sEnd, quality, show.title, forceDownload, show.type)
    else:
        episodes = show.getSeasons()[sStart].getEpisodes()
        try:
            eStart, eEnd, eIsRanged = getRange(episodeNumber)
            if (eStart < 0):
                return False
        except:
            return False
        ...
        StartEpisodesThreading(episodes, sStart, eStart, eEnd, quality, show.title, forceDownload, show.type)
